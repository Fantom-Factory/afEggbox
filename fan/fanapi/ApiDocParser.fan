...
//    this.pod = pod
  ...
  DocType parseType(Bool close := true)
  {
    try
    {
      ...
      // zero or more slots
      slots := Str:DocSlot[:] { caseInsensitive = true }	// SlimerDude - important for API resolving!!! See FandocLinkResolver
      while (true)
      {
        slot := parseSlot
        if (slot == null) break
...
//      return DocType(pod, attrs, typeRef, slots)
  ...
  ** Parse meta name/val pairs, facets, and fandoc section
  ...
  {
    facet2 := parseFacet
    if (facet2 == null) return
    acc := [facet2]
    while ((facet2 = parseFacet) != null) acc.add(facet2)
    attrs.facets = acc
  }
